...
class Card {
    ...
    constructor(cardId: string, name: string, collector: number, set: string, imageLink: string, imageAltText: string, description: string) {
        this.cardId = cardId;
        this.name = name;
...
function sortCards(order: "ascending" | "descending") {
    cards.sort((a, b) => {
        if (order === "ascending") {
...
function renderCards() {
    ...
    if (cardsContainer) {
        ...
        for (const card of cards) {
            ...
            setInfo.textContent = `${card.set} | ${card.getPaddedCollector()}`;
...
const dropdownList = document.querySelector<HTMLSelectElement>("#sort-by");
if (dropdownList) {
    dropdownList.addEventListener("change", (e) => {
        const target = e.target as HTMLSelectElement;
        sortCards(target.value as "ascending" | "descending");
        renderCards();
    });
}
